...
and check_var env x = match lookup env x with
    | Some t -> t
    | _ -> failwith "check_var: type expected"
and check_op o (t1: ty) (t2: ty) = match o, t1, t2 with
    | Add, Int, Int -> Int
    | Sub, Int, Int -> Int
    | Mul, Int, Int -> Int
    | Div, Int, Int -> Int
...
and check_fun t1 t2 = match t1 with
...
and check_con env e1 e2 e3 = 
    if check env e1 = Bool
    then 
        if check env e2 = check env e3
        then check env e2
        else failwith "check_con: e2 and e3 need to have the same type"
    else
        failwith "check_con: e1 must be bool"
...
and check_rec_let env f x t1 t2 e1 e2 = 
    ...
    match check (update envf x t1) e1 with
        | t2 -> check envf e2
