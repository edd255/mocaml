...
let rec tygr l =
    let (t, l) = ptygr l in tygr' t l
and tygr' t l = match l with
    | ARR::l -> let (t', l) = tygr l in (Arrow (t, t'), l)
    | _      -> (t, l)
and ptygr l = match l with
    | BOOL::l -> (Bool, l)
    | INT::l  -> (Int, l)
    | LP::l   -> (let (t, l) = tygr l in (t, verify RP l))
    | _       -> failwith "ptygr"
...
let rec exp l : exp * token list = match l with
    ...
    | LAM::VAR x::ARR::l ->
            let (e1, l) = exp l in
            (Lam (x, e1), l)
    | LAM::LP::VAR x::COL::l -> (* this caused a major pain... *)
            let (ty1, l) = tygr l in
    ...
    | LET::REC::VAR f::VAR x::EQ::l ->
            ...
            let (e2, l) = exp (verify IN l) in
            (Letrec (f, x, e1, e2), l)
    | LET::REC::VAR f::LP::VAR x::COL::l -> (* this caused a major pain... *)
            let (ty1, l) = tygr l in
...
and cexp' e l = match l with
    | LEQ::l -> let (e', l) = sexp l in (Oapp (Leq, e, e'), l)
    | EQ::l  -> let (e', l) = sexp l in (Oapp (Eq, e, e'), l) (* needed for = *)
    | _      -> (e, l)
and sexp l =
    let (e, l) = mexp l in sexp' e l
and sexp' e l = match l with
    | ADD::l -> let (e', l) = mexp l in sexp' (Oapp (Add, e, e')) l
    | SUB::l -> let (e', l) = mexp l in sexp' (Oapp (Sub, e, e')) l
    | _      -> (e, l)
and mexp l =
    let (e, l) = aexp l in mexp' e l
and mexp' e l = match l with
    | MUL::l -> let (e', l) = mexp l in mexp' (Oapp (Mul, e, e')) l
    | DIV::l -> let (e', l) = mexp l in mexp' (Oapp (Div, e, e')) l
    | _      -> (e, l)
and aexp l =
    let (e, l) = pexp l in aexp' e l
and aexp' e l = match l with
    | VAR x::_ -> let (e', l) = pexp l in aexp' (Fapp (e, e')) l
    | CON c::_ -> let (e', l) = pexp l in aexp' (Fapp (e, e')) l
    | LP::_    -> let (e', l) = pexp l in aexp' (Fapp (e, e')) l
    | _        -> (e, l)
